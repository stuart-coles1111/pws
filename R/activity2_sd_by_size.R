...
#'
#' Shows effect of sample size on standard deviation of estimate in Activity 2 of Playing With Statistics
#'
#'
...
#' @param transform_sd Transform Standard Deviation? (True/False)
...
activity2_sd_by_size <-
    function(n_throws = c(10, 25, 50, 100, 250, 500, 1000, 2500, 5000),
        ...
        for (i in 1:length(n_throws)) {
            scores <-
                activity2_data_sim(n_throws = n_throws[i],
                            ...
                            plot = FALSE)$score
            s[i] <-
                activity2_resample_est(scores, nrep = nrep, plot = FALSE)[["sd"]]
        }
        df <-
            data.frame(
                n_throws = n_throws,
        ...
        if (!transform_sd) {
            ggplot2::ggplot(data = df, ggplot2::aes(n_throws, sd)) +
                ggplot2::geom_point(colour = "steelblue") +
                ggplot2::labs(x = "Number of throws", y = "Standard deviation")
        }
        else{
            if (!log_trans) {
                ggplot2::ggplot(data = df, ggplot2::aes(n_throws, s)) +
                    ...
                    ggplot2::labs(x = "Number of throws", y = "Transformed standard deviation")
            }
            else{
                ggplot2::ggplot(data = df, ggplot2::aes(n_throws, s)) +
                    ggplot2:: geom_point(colour = "steelblue") +
                    ...
                    ggplot2::labs(x = "Number of throws (log scale)", y = "Transformed standard deviation (log scale)")
                )
            }
        }
    }
