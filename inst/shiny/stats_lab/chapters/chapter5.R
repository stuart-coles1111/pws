...
chapter5_ui <- function(id){
    ...
    chapter_page_ui(
        ...
        # =====================================================
        # OVERVIEW TAB (main content switch)
        # =====================================================
        ...
        # =====================================================
        # CODE TAB (optional teaching slot)
        # =====================================================
        ...
        code = tagList(
            div(
                class = "card-style",
                h3("Generated code"),
                p("This chapter combines simulation (binomial inference) and linear regression modelling.")
            )
        ),
        ...
        results = tagList(
            div(
                class = "card-style",
                h3("Results"),
                p("Run the simulations or regression to see outputs here.")
            )
        ),

        # =====================================================
        # LEARN TAB (conceptual explanation)
        # =====================================================
        ...
        learn = tagList(
            div(
                ...
                h3("Key ideas"),
                tags$ul(
                    tags$li("Bootstrap estimation of uncertainty"),
                    tags$li("Confidence intervals via normal approximation"),
                    tags$li("Simple linear regression inference")
                )
            )
        )
    )
...
# =========================================================
# SERVER (unchanged logic, just kept intact)
# =========================================================
...
chapter5_server <- function(id){
    ...
    moduleServer(id, function(input, output, session){
        ...
        output$bootstrap_plot <- renderPlot({
            ...
            if (!is.null(rv$ci)) {
                p +
                    geom_vline(xintercept = rv$ci, color = pal_red, linewidth = 1.1)
            } else {
                p
            }
        })
        ...
        output$reg_plot <- renderPlot({
            ...
            ggplot(df, aes(points_half1, points_half2)) +
                geom_point(color = pal_blue) +
                geom_line(data = plot_df, aes(points_half1, fit),
                          color = pal_lav, linewidth = 1.2,
                          inherit.aes = FALSE) +
                geom_ribbon(data = plot_df,
                            aes(points_half1, ymin = lwr, ymax = upr),
                            alpha = 0.2, fill = pal_lav,
                            inherit.aes = FALSE) +
                theme_minimal()
        })
        ...
        output$reg_results <- renderUI({
            ...
            HTML(paste0(
                ...
                "<p><b>Prediction:</b> ", round(pr[1, "fit"], 3), "</p>",
                "</div>"
            ))
        })
    })
